...
This page is a collection of useful commands or one-liners collected
through the years. They are sorted in general categories, but I've
attempted to describe them as well as I can so hopefully CTRL+F will
work decently if you know what you're looking for.
...
Note that this prints `localtime()` which depends on your system, and
looks for exactly 10 numbers.

If you have a log that begins with a very specific epoch timestamp
...
If you're on a system without jq but you're just looking to make a json
blob readable, you can use python's json.tool instead:
...
If you know that the timestamp is 14 characters long, this will cut out
the timestamp and show you a count of the worst offenders:
...
## Time travel with systemd

If your testing depends on the system clock being wrong, you can disable
NTP and set it to whatever like this:
...
- `JSON_DATA_OBJECT`: What you want to send to the API, in this case
  assumed JSON. If you need certain fields to be random, you could fill
  them with `$(cat /proc/sys/kernel/random/uuid)` for example.
...
If you need to do a `POST` or some other type of request with no data to
apply the change, just pass `-X POST` instead of `-d`ata.

#### Generate Nagios config file

To create a bunch of blocks with the expected syntax (here it's nagios),
you can do this to simply test lots of hosts:
...
How do you sort this alphabetically by `object{}`, not by line, so that
"alpha" appears before "gamma", but keeps its contents? There's always a
...
file, like `§`. This can be done in `vim` with:

```plain
:%g/.*{$/;/^}$/s/\n/§/
```

This replaces every newline with the placeholder `§`, and gives us:
...
Note that this only works if `}` also has a newline after it. You can
now sort it normally:

```plain
:%sort
```

When it's all sorted, restore the newlines by replacing the `§`
characters:
...
When bandwidth isn't the issue and you don't want to use up any space on
the remote system, you may want to do the compression yourself locally
and stream the tarball. If `sudo` is required, you can put it in a
...
You can also do the same only for a specific subset of files, for
example all log files:
...
If you're going to be throwing logs across the internet, it's probably a
good idea to encrypt the file. One way to do this is:
...
It will ask you for a passphrase. To decrypt, use the `-d` flag in an
otherwise very similar command:
...
Enter your passphrase and you'll have the decrypted file ready for
decompression.

## Bash

Since `!!` refers to the previous command, you can re-run a failed
...
So-called _brace expansion_ is commonly used to move or copy one or many
files as backups:
...
Use [shellcheck](https://github.com/koalaman/shellcheck) when writing
scripts, available as an extension in most editors. The [Google Shell
Style Guide](https://google.github.io/styleguide/shellguide.html) isn't
bad either.

## Product-specific: Salt-stack

If you don't use [salt](https://saltproject.io/whats-saltstack/), you
...
Wrangle this string to make it into a salt-compatible list
